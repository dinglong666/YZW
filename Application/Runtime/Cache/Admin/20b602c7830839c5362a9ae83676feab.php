		...
		<div class="main ">
			...
			<div class="row">
			            ...
			            <div class="panel-heading">
			                <a href="<?php echo U('Admin/adminlist');?>"><h2><i class="fa fa-indent red"></i><strong>返回列表</strong></h2></a>
			            </div>
						<div class="panel-body" style="width:50%;margin-left:20%">
							<form action="<?php echo U('Admin/admindit');?>" method="post" enctype="multipart/form-data" class="form-horizontal ">
				             	...
				             	<div class="form-group">
				                    <label class="col-md-3 control-label" for="text-input">确认新密码：</label>
				                    <div class="col-md-9">
				                        <input type="password" name="qpassword" class="form-control" placeholder="请输入确认密码">
				                    </div>
				                </div>
                                <?php if($role['id'] != 1): ?><div class="form-group">
                                        <label class="col-md-3 control-label" for="text-input">请选择权限：</label>
                                        <div class="col-md-9">
                                            <select name="qx">
                                                <?php if(is_array($qx)): $i = 0; $__LIST__ = $qx;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$x): $mod = ($i % 2 );++$i;?><option value="<?php echo ($x[role_id]); ?>"  <?php if($x['role_id'] == $role['role_id']): ?>selected<?php endif; ?> ><?php echo ($x['role_name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>    
                                            </select>
                                        </div>
                                    </div><?php endif; ?>
