...
<!-- 					<div class="copyright text-center">
		...
		<div class="main ">
			...
			<div class="row">
			            ...
			            <div class="panel-heading">
			                <a href="<?php echo U('Admin/adminPower');?>"><h2><i class="fa fa-indent red"></i><strong>返回列表</strong></h2></a>
			            </div>
						<div class="panel-body" style="width:50%;margin-left:20%">
							<form action="<?php echo U('Admin/adminPower_up');?>" method="post" enctype="multipart/form-data" class="form-horizontal ">
								<div class="form-group">
				                    <label class="col-md-3 control-label" for="text-input">权限名称：</label>
				                    <div class="col-md-9">
				                        <input type="text" name="pname" value="<?php echo ($find['role_name']); ?>" class="form-control" >
				                    </div>
				                </div>
				           		<div class="form-group">
				                    <label class="col-md-3 control-label" for="text-input">权限描述：</label>
				                    <div class="col-md-9">
				                        <input type="text" name="pdesc" value="<?php echo ($find['role_desc']); ?>" class="form-control" >
				                    </div>
				                </div>
				             	<div class="form-group">
				                    <label class="col-md-6 control-label" for="text-input" style='font-size:25px'>权限分配 <input id='qx' type="checkbox"  /></label><br />
				                </div>
                            <?php if(is_array($qx)): $ke = 0; $__LIST__ = $qx;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$q): $mod = ($ke % 2 );++$ke;?><div class="form-group">
                                <?php if(is_array($q)): $i = 0; $__LIST__ = array_slice($q,0,1,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$x): $mod = ($i % 2 );++$i;?><label class="col-md-3 control-label" for="text-input"><?php echo ($x); ?><input class="qx2" type="checkbox"  /></label><br /><?php endforeach; endif; else: echo "" ;endif; ?>
                                    <div class="col-md-9">
                                    <?php if(is_array($q)): $k = 0; $__LIST__ = array_slice($q,1,30,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$x): $mod = ($k % 2 );++$k; echo ($x); ?><input type="checkbox" value="<?php echo ($ke); echo ($k); ?>" name='qx[]' <?php echo tf_qx($ke.$k,$find['act_list']);?> />&nbsp;&nbsp;&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>    
                                    </div>
    ...
    $('#qx').click(function()
    {
        $('[type=checkbox]').prop('checked',$('#qx').prop('checked'));            
    });
    

    $('.qx2').click(function()
    {
        $(this).parent().next().next().children('input').prop('checked',$(this).prop('checked'));
    });
